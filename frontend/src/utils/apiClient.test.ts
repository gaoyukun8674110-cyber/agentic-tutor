...
describe('apiClient localization and auth refresh', () => {
  ...
  it('returns Chinese auth errors when the stored language is zh', () => {
    ...
    expect(getUserFacingError(new ApiError('Missing access token', 401, 'unauthorized'))).toBe('需要身份验证。请先登录。');
    expect(getUserFacingError(new ApiError('User not allowed', 403, 'forbidden'))).toBe('你没有权限执行此操作。');
  });
  ...
  it('refreshes once and replays a protected request after 401', async () => {
    ...
    const fetchSpy = vi
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ detail: { code: 'token_expired', user_message: 'Access token expired' } }), { status: 401 }))
    ...
    expect((fetchSpy.mock.calls[2][1]?.headers as Headers).get('Authorization')).toBe('Bearer fresh-token');
  });
  ...
  it('uses one refresh request for concurrent 401 responses', async () => {
    ...
    expect(fetchSpy.mock.calls.filter((call) => String(call[0]).includes('/api/auth/refresh'))).toHaveLength(1);
  });
});
